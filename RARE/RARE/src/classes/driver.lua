...
function Driver:initialize(carIndex)
	...
	self.fuelPerLap = (sim.trackLengthM / 1000) / (self.fuelCons - (self.fuelCons * 0.1))
	self.brakeTorque = ac.INIConfig.carData(self.index, "brakes.ini"):get("DATA", "MAX_TORQUE", 0) - (ac.INIConfig.carData(self.index, "brakes.ini"):get("DATA", "MAX_TORQUE", 0)*2)
	
	log("[" .. self.index .. "] " .. self.name .. " [" .. self.fuelCons .. "/" .. self.fuelPerLap .. "] initialized")
...
function Driver:getFuelPerLap()
	return (sim.trackLengthM / 1000) / (self.fuelCons - (self.fuelCons * 0.1))
end
...
function Driver:update(dt)
	...
	if self.isSafetyCarAllowed then
		...
		if not self.car.isInPit then
			if self.car.speedKmh < 0.1 then 
				if self.car.gas < 0.2 then -- if (not)inPit + stopped + no throttle
					self.crashed = true
				end
			end
		else
