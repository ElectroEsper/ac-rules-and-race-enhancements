...
local function tyresTreeNode(driver)
	ui.treeNode("[TYRES]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
		...
		ui.inLineBulletText("Brake Bias", math.round(driver.car.brakeBias, 2),space)
		ui.inLineBulletText("Tyre Speed [FL]", math.round(driver.car.wheels[0].angularSpeed, 0),space)
		ui.inLineBulletText("Tyre Speed [RL]", math.round(driver.car.wheels[2].angularSpeed, 0),space)
		ui.inLineBulletText("Tyre Speed [FR]", math.round(driver.car.wheels[1].angularSpeed, 0),space)
		ui.inLineBulletText("Tyre Speed [RR]", math.round(driver.car.wheels[3].angularSpeed, 0),space)
	end)
end
...
local function sessionTreeNode(sim, rc, driver)
	ui.treeNode(
		...
		function()
			...
			--ui.inLineBulletText("SC Allowed" .. .. " seconds", ui.upperBool(rc.sc.SAFETYCAR_ALLOWEDAFTER), space)
			...
			--ui.inLineBulletText("SC On 1st Lap", ui.upperBool(sc.firstLap()), space)
			ui.inLineBulletText("SC Deployed", ui.upperBool(sc.isDeployed()), space)
			--ui.inLineBulletText("SC Inpit", ui.upperBool(sc.inPit()), space)
			--ui.inLineBulletText("SC Leader Behind", ui.upperBool(sc.leaderBehind()),space)
			--ui.inLineBulletText("SC Called-in", ui.upperBool(sc.calledIn()), space)
			--ui.inLineBulletText("SC Coming-in", ui.upperBool(sc.comingIn()), space)
			ui.inLineBulletText("SC Completed Laps",sc.lapsCompleted() .. "/" .. sc.lapsMax(), space)
			--ui.inLineBulletText("SC Pack Pre-Restart", ui.upperBool(sc.preRestartCheck()), space)
			ui.inLineBulletText("SC Pack Restart Spline",sc.restartCheck(), space)
			ui.inLineBulletText("Crashed car", sc.crashCount(), space)
